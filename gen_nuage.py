...
def etoile(x, y, s):
    return (f'<path d="M {x} {y-s} Q {x+s*0.22} {y-s*0.22} {x+s} {y} '
            ...
            f'Q {x-s*0.22} {y-s*0.22} {x} {y-s} Z" fill="{CORPS}" opacity="0.95"/>')

ETOILES = etoile(96, 96, 11) + etoile(434, 72, 8) + etoile(458, 156, 6)
...
print(f"{len(ETATS) * 2} SVG générés (v2)")
